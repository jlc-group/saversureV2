...
function DefaultHomeContent() {
  ...
  const quickLinks = [
    { href: "/scan", icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M9.5 6.5v3h-3v-3h3M11 5H5v6h6V5zm-1.5 9.5v3h-3v-3h3M11 13H5v6h6v-6zm6.5-6.5v3h-3v-3h3M19 5h-6v6h6V5z" /></svg>, bg: "bg-gradient-to-br from-green-100 to-teal-50 text-[var(--jh-green)]", title: "สะสมแต้ม" },
    { href: "/rewards", icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M20 6h-2.18c.11-.31.18-.65.18-1a3 3 0 0 0-3-3c-1.05 0-1.96.54-2.5 1.35l-.5.67-.5-.68C10.96 2.54 10.05 2 9 2a3 3 0 0 0-3 3c0 .35.07.69.18 1H4c-1.11 0-1.99.89-1.99 2L2 19c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V8c0-1.11-.89-2-2-2z" /></svg>, bg: "bg-gradient-to-br from-amber-100 to-yellow-50 text-amber-600", title: "แลกรางวัล" },
    { href: "/history", icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M13 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18zm1 10h-5V7h2v4h3v2z" /></svg>, bg: "bg-gradient-to-br from-blue-100 to-indigo-50 text-blue-600", title: "ประวัติ" },
    { href: "/news", icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 14H4V6h16v12zM6 10h2v2H6v-2zm0 4h8v2H6v-2zm10 0h2v2h-2v-2zm-6-4h8v2h-8v-2z" /></svg>, bg: "bg-gradient-to-br from-purple-100 to-pink-50 text-purple-600", title: "ข่าวสาร" },
  ];
  ...
  return (
    <>
        ...
        <div className="relative">
          ...
          <div className="mt-5 flex items-center justify-between rounded-xl bg-white/15 px-4 py-3 ring-1 ring-white/20">
            <div>
              ...
              <p className="text-3xl font-bold leading-tight animate-shimmer">{(primaryBalance?.balance ?? 0).toLocaleString()}</p>
            ...
            {!loggedIn ? (
              <Link href="/login" className="rounded-full bg-white px-5 py-2 text-sm font-bold text-[var(--jh-green-dark)] animate-pulse-glow">เข้าสู่ระบบ</Link>
            ) : (
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-3">
            <div className="grid grid-cols-4 gap-1 stagger-children">
              {quickLinks.map((item) => (
                <Link key={item.href} href={item.href} className="card-playful flex flex-col items-center gap-1.5 rounded-xl py-3 transition hover:scale-105 active:bg-muted">
        ...
        <Card className="border-0 shadow-sm overflow-hidden">
          <CardContent className="p-0">
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-5 relative">
              <div className="absolute right-3 top-3 text-4xl opacity-30 animate-wiggle">🎉</div>
      ...
      {/* Features */}
      <div className="px-4 mt-4 space-y-2 animate-slide-up">
        ...
        {[
          ...
          <Card key={f.title} className="border-0 shadow-sm card-playful">
            <CardContent className="p-4 flex items-center gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-green-100 to-teal-50 text-[var(--jh-green)]">{f.icon}</div>
