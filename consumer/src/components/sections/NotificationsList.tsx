...
interface NotificationsListProps {
  ...
  time_minute_suffix?: string;
  time_hour_suffix?: string;
  time_day_suffix?: string;
}
...
export default function NotificationsList({
  ...
  function timeAgo(dateStr: string): string {
    ...
    if (diff < 60) return time_just_now;
    if (diff < 3600) return `${Math.floor(diff / 60)}${time_minute_suffix}`;
    if (diff < 86400) return `${Math.floor(diff / 3600)}${time_hour_suffix}`;
    if (diff < 2592000) return `${Math.floor(diff / 86400)}${time_day_suffix}`;
  ...
  if (!loggedIn) {
    return (
      <div className="max-w-[480px] mx-auto px-5 py-16 text-center">
        <p className="text-[14px] text-[var(--on-surface-variant)] mb-4">
          {login_required_text}
        </p>
        <Link
          href={login_href}
  ...
  return (
    <div className="max-w-[480px] mx-auto px-5 py-4">
      {/* Top action bar (unread badge + mark-all) */}
      {!loading && notifications.length > 0 && (
        <div className="flex items-center justify-between mb-3">
          {unread > 0 ? (
            <span className="min-w-[20px] h-5 px-2 rounded-full bg-[var(--primary)] text-white text-[11px] font-medium flex items-center justify-center">
              {unread > 99 ? "99+" : unread}
            </span>
          ) : (
            <span />
          ...
          >
            {markingAll ? marking_label : mark_all_label}
          </button>
        </div>
      )}
      ...
      {loading ? (
        <div className="flex justify-center py-16">
          <svg
            className="animate-spin w-6 h-6 text-[var(--primary)]"
            viewBox="0 0 24 24"
            fill="none"
          >
            <circle
