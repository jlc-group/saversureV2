...
interface MissionsTabsListProps {
  show_login_cta?: boolean;
  login_cta_text?: string;
  empty_all_text?: string;
  empty_completed_text?: string;
  tabs?: TabDef[];
  default_tab?: string;
  show_claim_modal?: boolean;
}
...
export default function MissionsTabsList({
  show_login_cta = true,
...
}: MissionsTabsListProps) {
  const effectiveTabs = tabs && tabs.length > 0 ? tabs : DEFAULT_TABS;
  const [missions, setMissions] = useState<Mission[]>([]);
  const [activeTab, setActiveTab] = useState<string>(default_tab || effectiveTabs[0]?.id || "all");
    ...
    const load = async () => {
      ...
      try {
        const [missionsRes, progressRes] = await Promise.all([
          ...
          loggedIn
            ? api
                .get<{ data?: MissionProgress[] } | MissionProgress[]>("/api/v1/my/missions")
                .catch(() => null)
            : null,
        ]);

        const missionsList = Array.isArray(missionsRes)
          ? missionsRes
          : missionsRes.data ?? [];
        ...
        if (progressRes) {
          ...
          list.forEach((p: MissionProgress) => {
            map[p.mission_id] = p;
          });
          setProgressMap(map);
        }
  ...
  const handleClaim = async (e: React.MouseEvent, mission: Mission) => {
    ...
    setProgressMap((prev) => ({
      ...
      [mission.id]: {
        ...
        progress: prev[mission.id]?.progress ?? 0,
        completed_at: mounted ? new Date().toISOString() : null,
      },
    }));
  };

  const filterMissionsForTab = (tabId: string) => {
    if (tabId === "completed") {
      return missions.filter((m) => !!progressMap[m.id]?.completed_at);
    }
    // default: "all" = not completed (ยังไม่สำเร็จ)
    return missions.filter((m) => !progressMap[m.id]?.completed_at);
  };

  const displayMissions = filterMissionsForTab(activeTab);
  const emptyText = activeTab === "completed" ? empty_completed_text : empty_all_text;

  return (
    <>
      {show_login_cta && !loggedIn && (
        <div className="px-5 -mt-6 relative z-10">
          <div className="bg-white rounded-2xl shadow-md p-4 text-center">
            <p className="text-[14px] text-muted-foreground mb-3">{login_cta_text}</p>
            <Link
              href="/login"
      ...
      <div className="px-5 mt-2">
        {loading ? (
          ...
          <EmptyState
            ...
            subtitle={
              activeTab === "completed"
                ? "มาเริ่มทำขุมทรัพย์แรกกันเถอะ"
                : "ภารกิจใหม่กำลังจะมาเร็วๆ นี้"
            }
          />
        ) : (
          <div className="space-y-3 stagger-children">
            {displayMissions.map((m, index) => {
              ...
              const pct =
                m.target > 0 ? Math.min(100, Math.round((progress / m.target) * 100)) : 0;
              ...
              const daysLeft =
                m.end_date && mounted
                  ...
                  : null;
              const timeText =
                daysLeft !== null ? (daysLeft > 0 ? `เหลืออีก ${daysLeft} วัน` : "หมดเวลา") : "ตลอดปี";
                ...
                >
                  <div className="flex p-4 gap-3 items-center">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      ...
                      <h3 className="text-[15px] font-extrabold text-gray-800 truncate tracking-tight">
                        {m.title}
                      </h3>
                      {m.description && (
                      ...
                      {completed ? (
                        ...
                        <div className="mt-3.5">
                          <div className="relative h-2.5 bg-gray-100 rounded-full flex items-center mx-1">
                            <div
                              className="absolute left-0 top-0 bottom-0 bg-[#4caf50] rounded-full transition-all duration-700"
                              style={{ width: `${pct}%` }}
                            ></div>
                            <div className="absolute left-0 right-0 flex justify-between px-[0px] -mx-1.5">
                              {Array.from({ length: m.target <= 12 ? m.target + 1 : 5 }).map(
                                (_, i, arr) => {
                                  const segments = arr.length - 1;
                                  const dotPct = (i / segments) * 100;
                                  const isColored = pct >= dotPct;
                                  const isFarthestColor =
                                    isColored &&
                                    (i === arr.length - 1 ||
                                  ...
                                  return (
                                    <div
                                      key={i}
                                      className={`w-3.5 h-3.5 rounded-full border-[2.5px] flex items-center justify-center z-10 transition-colors duration-500 ${
                          ...
                          <div className="flex justify-between items-center text-[12px] font-bold mt-2.5 px-0.5">
                            ...
                            <span className="text-[#f44336] text-[11px] font-bold">
                              {timeText}
                            </span>
                          </div>
                        </div>
                    ...
                    <div
                      className={`w-[72px] h-[72px] shrink-0 rounded-full border-[3px] ${badgeTheme.border} bg-white flex flex-col items-center justify-center shadow-[0_2px_8px_rgba(0,0,0,0.04)] ml-1`}
                    >
                      {m.reward_type === "badge" ? (
                        <>
                          <span
                            className={`font-extrabold text-[13px] leading-tight text-center px-1 ${badgeTheme.text}`}
                          >
                            Badge
                            <br />
                            ไอเทม
                        ...
                        <>
                          <span
                            className={`font-black text-[15px] leading-none ${badgeTheme.text}`}
                          >
                            {m.reward_points ?? 0}
                            <span className="text-[10px] ml-0.5">แต้ม</span>
                          </span>
      ...
      {show_claim_modal && claimedMission && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-5 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl w-full max-w-[320px] p-6 text-center shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] animate-bounce-in relative overflow-hidden">
            ...
            <div className="relative mx-auto w-24 h-24 mb-4">
              ...
              <div
                className="absolute inset-0 flex items-center justify-center text-6xl animate-wiggle"
            ...
            <h2 className="text-[24px] font-black text-gray-800 mb-2 relative z-10">
              ยินดีด้วย!
            </h2>
            <p className="text-[15px] text-gray-600 mb-5 relative z-10 leading-snug">
              คุณทำภารกิจ <br />
              <span className="font-extrabold text-[var(--jh-green)]">
            ...
            <div className="bg-orange-50 rounded-2xl p-4 mb-6 border border-orange-100 relative z-10">
              ...
              <div className="text-[28px] font-black text-orange-500 leading-none drop-shadow-sm">
                {claimedMission.reward_type === "points"
                  ? `+${claimedMission.reward_points} แต้ม`
                  : "Badge ใหม่!"}
              </div>
            </div>
